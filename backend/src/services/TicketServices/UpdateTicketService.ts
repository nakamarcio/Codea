...
const UpdateTicketService = async ({
  ...
  try {
    ...
    if (["pending", "open"].indexOf(status) > -1) {
      const ttUpdate = {
        whatsappId: ticket.whatsappId,
        userId: status === "open" ? ticket.userId : null,
        startedAt: status === "open" ? moment().toDate() : null,
      };

      if (status === "open") {
        ttUpdate["ratingAt"] = null;
        ttUpdate["rated"] = false;
      } else if (status === "pending") {
        ttUpdate["queuedAt"] = moment().toDate();
      }

      ticketTraking.update(ttUpdate);
